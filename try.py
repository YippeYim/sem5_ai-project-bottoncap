...
def process_yolov8_output(output, conf_threshold=0.25, iou_threshold=0.45, img_size=640):
    preds = np.squeeze(output).T  # (8400, num_classes+5)
    boxes, scores, class_ids = [], [], []

    for pred in preds:
        # adjust according to number of classes
        x, y, w, h, conf, *cls_scores = pred
...
img_size = input_details[0]['shape'][1]  # e.g. 640
...
while True:
    ...
    for r in results:
        ...
        cv2.putText(frame_resized, f"{r['class']} {r['confidence']:.2f}",
                    (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

    cv2.imshow("YOLOv8 PiCam Detection", frame_resized)
